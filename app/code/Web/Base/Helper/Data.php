...
class Data extends \Magento\Framework\App\Helper\AbstractHelper 
{
    ...
    public function getProductByCategoryId($categoryId) {
        ...
        $categoryProducts = $category->getProductCollection()->addAttributeToSelect('*')->addAttributeToFilter('type_id', array('eq' => 'configurable'));
        /*$categoryProducts->getSelect()->joinInner(
            ['stock' => $this->stockStatusResource->getMainTable()],
            'stock.product_id = entity.entity_id',
            ['stock.stock_status']
        );*/
        $categoryProducts->setVisibility($this->_catalogProductVisibility->getVisibleInCatalogIds())->setPageSize(3);
    ...
    {
        /** @var \Magento\Catalog\Model\Product $loadedProduct */
        $loadedProduct = $this->productFactory->create()->loadByAttribute('entity_id', $product->getId());

        if((double)$loadedProduct->getData('product_shipping_cost')){
            return false;
        } else {
            return true;
        }
    ...
    public function loadStoreProduct($productId)
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $product = $objectManager->get('Magento\Catalog\Model\Product')->load($productId);
        if($product)        
