    ...
    \Magento\Shipping\Model\Carrier\CarrierInterface
{
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        foreach ($rateArray as $rate) {
            if (!empty($rate) && $rate['price'] >= 0) {
                ...
                $method->setCarrierTitle(__($rate['shipping_method']));

                $method->setMethod('matrixrate_' . $rate['pk']);
                $newTitle = $this->getRelatedTitle($rate['shipping_method'],explode(',', $this->getConfigData('title')));
     ...
     * @return array
     */
    public function getRelatedTitle($method,$title)
    {
        $newTitle = '';

        if(!empty($title)){
            foreach ($title as $value) {
                $titleMethod = explode('|', $value);
                if(isset($titleMethod[0])){
                    $titleMethod = $titleMethod[0];
                }
                if($titleMethod == $method){
                    $newTitle = explode('|', $value);
                    if(isset($newTitle[1])){
                        $newTitle = $newTitle[1];
                    }
                }
