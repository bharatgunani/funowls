...
class LayoutGenerateBlocksAfterBaseSeoChangesObserver implements ObserverInterface
{
    ...
    const ROBOTS_STRATEGY_INDEX_NOFOLLOW = 'INDEX,NOFOLLOW';
    /**#@-*/
    ...
    private function processRobotsStrategy($query, $pathInfo)
    {
        if (
            ...
            || preg_match('/^\/weltpixel_quickview\/catalog_product\/view/', $pathInfo)
        ) {
    ...
    private function processCanonical()
    {
        ...
        if (in_array($this->request->getModuleName(), ['cms'])) {

            if ($canonicalGroup && $canonicalGroup->has($currentUrl) && ($currentUrl != $canonicalUrl)) {
