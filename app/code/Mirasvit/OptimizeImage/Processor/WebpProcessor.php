...
class WebpProcessor implements OutputProcessorInterface
{
    ...
    private function replaceCallback(array $match)
    {
        ...
        $defaultSource    = $this->getDefaultSource($path, $match[2], $classes, $webpPath);
        $responsiveSource = $this->getResponsiveSource($path, $match[2], $classes);
     ...
     * @return string
     */
    private function getDefaultSource($path, $imageUrl, $classes, $webpPath)
    {
        $ext             = $this->config->getFileExtension($path);
        ...
        return '<source srcset="' . $webpUrl . '" type="image/webp"' . $classes . '/>';
    }
